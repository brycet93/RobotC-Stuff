...
void PushEggIntoNest(int nest);

task main()
{
	int base = SensorValue[S2]; //This calibrates which direction the base is on the gyro.
	MoveOutOfBase();
...
void MoveForward()
{
	setMotorSpeed(motorB, 50);		//Set the leftMotor (motor1) to half power (50)
	setMotorSpeed(motorC, 50);  	//Set the rightMotor (motor6) to half power (50)
	sleep(500);						//Wait for a random amount of time before continuing on in the program.
}
...
void WanderRight()
{
	...
	setMotorSpeed(motorC, 50);  	//Set the rightMotor (motor6) to full power forward (100)
	sleep(rand()%500);						//Wait a random amount of time before continuing on in the program.
}

void WanderLeft()
{
	setMotorSpeed(motorB, 50);		//Set the leftMotor (motor1) to full power forward (100)
...
void PushEggTowardsNest(int nest)
{
	while((abs(SensorValue[S2])%360)!=(abs(nest-180)%360))
	{
		setMotorSpeed(motorB, 20);
		setMotorSpeed(motorC, -20);
	}
...
void PushEggIntoNest(int nest)
{
	...
	{
		setMotorSpeed(motorB, 50);	//Set the leftMotor (motor1) to high power forward (80)
		setMotorSpeed(motorC, 50);	//Set the rightMotor (motor6) to high power forward (80)
  }
  ...
  sleep(500);
	...
	while((abs(SensorValue[S2])%360)!=(abs(nest)%360))
	{
		setMotorSpeed(motorB, 20);
		setMotorSpeed(motorC, -20);
	}
